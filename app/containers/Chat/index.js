...
import {SectionList, Image, Text, View, TouchableOpacity, Modal} from 'react-native';
...
function Chat({me, message, date, reaction, onLongPress, onDelete, isDeleted, attachmentUrl, isPinned, onPin, onUnpin}) {
			...
			<TouchableOpacity onLongPress={onLongPress}>
				<View className={`${me ? 'flex-row-reverse' : 'flex-row'} items-end mb-1`}>
					...
					<View className={`p-3 rounded-2xl max-w-[85%] ${me ? 'bg-main' : 'bg-light'}`}>
						{attachmentUrl && (
							<TouchableOpacity onPress={() => attachmentUrl.endsWith('.mp4') ? null : setModalVisible(true)} className="mb-2">
